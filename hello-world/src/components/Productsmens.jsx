...
export const ProductsMens = () =>{
    ...
    let womens = useSelector((state)=>state.users);
 ...
 let title;
  ...
  const getForm = () => {
        //  dispatch(getwomensLoading());
         fetch(`http://localhost:3002/men?_page=${page}&_limit=10`)
         .then((d) => d.json())
         .then((res) => {
...
<select name="productType" id="productType"  className="filterButtons">
 ...
 <select name="gender" id="gender"  onClick={showGender} className="filterButtons">
...
<button  className="filterButtons">CLEAR FILTER</button>
                 ...
                 <div className="products display-products" > {
                  title.map((e)=>(
                      <div >
                         <div key ={e.id}className="product" id="product" onClick={()=>{productDetails(e)}} >
                            <div><Link to ={`/details/${e.id}`}>
                              <div className="img" onMouseLeave={()=>{change1(e)}} onMouseOver={()=>change(e)}>{(<img src={e.gallary[0]} alt="demo"/>)}</div>
                              <div>{e.price}</div>
